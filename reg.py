...
def _is_staff(user) -> bool:
    ...
    return role in ("admin", "sadmin", "trainer") or bool(getattr(user, "isCurator", False))
...
async def send_reg(message: Message, text: str, *, reply_markup=None, parse_mode=None):
...
async def ask_input(message: Message, text: str, *, markdown: bool = True):
    kb = empty_kb()
    if markdown:
        await send_reg(message, text, parse_mode="Markdown", reply_markup=kb)
    else:
        await send_reg(message, text, reply_markup=kb)
...
async def accept_offer(message: Message, state: FSMContext):
    ...
    await state.set_state(ClientFSM.first_name)
    await send_reg(message, "❗ Для регистрации клиента заполните данные и нажмите «✅ Зарегистрироваться».", reply_markup=reg_kb)
    await _preview_form(message, state)
    await ask_input(message, "✏️ Введите ваше *имя*:")
...
async def decline_offer(message: Message, state: FSMContext):
    await state.clear()
    await send_reg(message, "❌ Регистрация отменена.", reply_markup=main_kb())
...
async def client_email(message: Message, state: FSMContext):
    ...
    if not _email_ok(v):
        await send_reg(message, "Неверный e-mail. Пример: user@example.com", reply_markup=empty_kb())
        return
...
async def client_phone(message: Message, state: FSMContext):
    v = message.text.strip()
    if not _phone_ok(v):
        await send_reg(message, "Неверный телефон. Пример: +79991234567", reply_markup=empty_kb())
        return
...
async def client_height(message: Message, state: FSMContext):
    try:
        ...
        await send_reg(message, "Введите рост в см (например: 180)", reply_markup=empty_kb())
        return
...
async def client_weight(message: Message, state: FSMContext):
    try:
        ...
        await send_reg(message, "Введите вес в кг (например: 82.5)", reply_markup=empty_kb())
        return
...
async def client_age(message: Message, state: FSMContext):
    try:
        ...
        await send_reg(message, "Введите возраст целым числом (например: 29)", reply_markup=empty_kb())
        return
...
async def reg_start(message: Message, state: FSMContext):
    ...
    if cur and cur.startswith("ClientFSM"):
        await send_reg(message, "Продолжайте ввод данных ⬆️")
        return
...
async def reg_cancel(message: Message, state: FSMContext):
    await state.clear()
    await send_reg(message, "❌ Отменено. Вы в главном меню.", reply_markup=main_kb())
...
async def profile_open(message: Message):
    ...
    paid_until = ""
    if _tariff_active(u) and getattr(u, "tariffExpiresAt", None):
        paid_until = _format_date_only(u.tariffExpiresAt)
    paid_until_label = paid_until or "не оплачено"
    ...
    text = (
        ...
        f"Возраст: `{u.age or '-'}`\n"
        f"Текущий тариф: `{(u.tariffName if _tariff_active(u) else None) or 'не куплен'}`\n"
        f"Оплачено до: `{paid_until_label}`\n"
    )
...
async def client_first(message: Message, state: FSMContext):
    v = message.text
    if not _name_ok(v):
        await send_reg(message, "Имя должно содержать только буквы, 2–30 символов.", reply_markup=empty_kb())
        return
...
async def client_last(message: Message, state: FSMContext):
    v = message.text
    if not _name_ok(v):
        await send_reg(message, "Фамилия должна содержать только буквы, 2–30 символов.", reply_markup=empty_kb())
        return
    await state.update_data(last_name=_name_fix(v))
    await _preview_form(message, state)
    await state.set_state(ClientFSM.height_cm)
    await ask_input(message, PROMPT_BY_FIELD["height_cm"])
...
ORDER = ["first_name", "last_name", "height_cm", "weight_kg", "age"]
...
async def edit_age_set(message: Message, state: FSMContext):
    try:
        a = int(message.text.strip())
        if a < MIN_AGE or a > MAX_AGE: raise ValueError()
    except Exception:
